...
#define MEMC_METHOD_FETCH_OBJECT                                               \
	...
	if (!i_obj->obj) {	\
		char *name;	\
		zend_uint name_len;	\
		int dup = zend_get_object_classname(object, &name, &name_len TSRMLS_CC);	\
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "%s constructor was not called", name);	\
		if (!dup) {	\
			efree(name); \
		} \
		return;	\
...
typedef struct {
	...
	struct php_memc_internal {
		memcached_st *memc;
		// rescode is enum mecached_return
		int rescode;
...
static int php_memc_handle_error(struct php_memc_internal *memc_obj, memcached_return status TSRMLS_DC);
...
static PHP_METHOD(MEMCACHED_CLASS_NAME, __construct)
{
	zval *object = getThis();
	php_memc_t *i_obj;
	memcached_st *memc;
	...
	is_persistent = (persistent_id != NULL);

	if (persistent_id) {
		struct php_memc_internal *pi = NULL;
		zend_rsrc_list_entry *le;

		plist_key_len = spprintf(&plist_key, 0, "memcached:id=%s", persistent_id);
		plist_key_len += 1; // include the terminating null in the key

		if (plist_key == NULL) {
			php_error_docref(NULL TSRMLS_CC, E_ERROR, "could not allocate persistent list key");
			/* not reached */
		}

		if (zend_hash_find(&EG(persistent_list), plist_key, plist_key_len, (void *)&le) == SUCCESS) {
			if (le->type == php_memc_list_entry()) {
				pi = (struct php_memc_internal *)le->ptr;
			}
		}

		/*
		 * If the key is fround in the persistent list copy the pointer to it, and
		 * set suitable parameters and skip the constructor.
		 */
		if (pi) {
			skip_ctor = true;
	...
	struct php_memc_internal *pi = pemalloc(sizeof(*pi), is_persistent);
	if (pi == NULL) {
...
PHP_METHOD(MEMCACHED_CLASS_NAME, get)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getByKey)
{
...
static void php_memc_get_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool by_key)
{
	...
	i_obj->obj->rescode = MEMCACHED_SUCCESS;

	if (key_len == 0) {
		i_obj->obj->rescode = MEMCACHED_BAD_KEY_PROVIDED;
		RETURN_FROM_GET;
	}
	...
	if (cas_token) {
		 ...
		 */
		orig_cas_flag = memcached_behavior_get(i_obj->obj->memc, MEMCACHED_BEHAVIOR_SUPPORT_CAS);
		if (orig_cas_flag == 0) {
			memcached_behavior_set(i_obj->obj->memc, MEMCACHED_BEHAVIOR_SUPPORT_CAS, 1);
		}

		status = memcached_mget_by_key(i_obj->obj->memc, server_key, server_key_len, &key, &key_len, 1);

		if (php_memc_handle_error(i_obj->obj, status TSRMLS_CC) < 0) {
			RETURN_FROM_GET;
		}

		status = MEMCACHED_SUCCESS;
		memcached_result_create(i_obj->obj->memc, &result);

		if (memcached_fetch_result(i_obj->obj->memc, &result, &status) == NULL) {
		...
		if (php_memc_zval_from_payload(return_value, payload, payload_len, flags TSRMLS_CC) < 0) {
			memcached_result_free(&result);
			i_obj->obj->rescode = MEMC_RES_PAYLOAD_FAILURE;
			RETURN_FROM_GET;
		}
		 ...
		 */
		if (orig_cas_flag == 0) {
			memcached_behavior_set(i_obj->obj->memc, MEMCACHED_BEHAVIOR_SUPPORT_CAS, orig_cas_flag);
		}
		return;
		...
		status = memcached_mget_by_key(i_obj->obj->memc, server_key, server_key_len, &key, &key_len, 1);
		payload = memcached_fetch(i_obj->obj->memc, NULL, NULL, &payload_len, &flags, &status);
		/* This is for historical reasons */
		if (status == MEMCACHED_END)
			status = MEMCACHED_NOTFOUND;
		...
		(void)memcached_fetch(i_obj->obj->memc, NULL, NULL, &dummy_length, &dummy_flags, &dummy_status);
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getMulti)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getMultiByKey)
{
...
static void php_memc_getMulti_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool by_key)
{
	...
	status = memcached_mget_by_key(i_obj->obj->memc, server_key, server_key_len, mkeys, mkeys_len, i);
	...
	status = MEMCACHED_SUCCESS;
	memcached_result_create(i_obj->obj->memc, &result);
	while ((memcached_fetch_result(i_obj->obj->memc, &result, &status)) != NULL) {
	...
	if (status != MEMCACHED_END && php_memc_handle_error(i_obj->obj, status TSRMLS_CC) < 0) {
		zval_dtor(return_value);
		RETURN_FROM_GET;
	}
}
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getDelayed)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getDelayedByKey)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, fetch)
{
	char *res_key = NULL;
	size_t res_key_len = 0;
	char  *payload = NULL;
	...
	memcached_result_create(i_obj->obj->memc, &result);
	if ((memcached_fetch_result(i_obj->obj->memc, &result, &status)) == NULL) {
		php_memc_handle_error(i_obj->obj, status TSRMLS_CC);
		memcached_result_free(&result);
		RETURN_FROM_GET;
	}
...
PHP_METHOD(MEMCACHED_CLASS_NAME, fetchAll)
{
	char *res_key = NULL;
	size_t res_key_len = 0;
	char  *payload = NULL;
...
PHP_METHOD(MEMCACHED_CLASS_NAME, set)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, setByKey)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, setMulti)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, setMultiByKey)
{
...
static void php_memc_setMulti_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool by_key)
{
	...
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(entries));
		...
		flags = 0;
		if (i_obj->obj->compression) {
			flags |= MEMC_VAL_COMPRESSED;
		}

		payload = php_memc_zval_to_payload(*entry, &payload_len, &flags, i_obj->obj->serializer TSRMLS_CC);
		if (payload == NULL) {
			i_obj->obj->rescode = MEMC_RES_PAYLOAD_FAILURE;
			RETURN_FALSE;
		}
		...
		status = memcached_set_by_key(i_obj->obj->memc, server_key, server_key_len, str_key,
...
PHP_METHOD(MEMCACHED_CLASS_NAME, add)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, addByKey)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, append)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, appendByKey)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, prepend)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, prependByKey)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, replace)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, replaceByKey)
{
...
static void php_memc_store_impl(INTERNAL_FUNCTION_PARAMETERS, int op, zend_bool by_key)
{
	...
	payload = php_memc_zval_to_payload(value, &payload_len, &flags, i_obj->obj->serializer TSRMLS_CC);
	if (op == MEMC_OP_APPEND || op == MEMC_OP_PREPEND) {
		zval_ptr_dtor(&value);
	}
	if (payload == NULL) {
	...
	switch (op) {
		case MEMC_OP_SET:
			status = memcached_set_by_key(i_obj->obj->memc, server_key, server_key_len, key,
										  key_len, payload, payload_len, expiration, flags);
			break;

		case MEMC_OP_ADD:
			status = memcached_add_by_key(i_obj->obj->memc, server_key, server_key_len, key,
										  key_len, payload, payload_len, expiration, flags);
			break;

		case MEMC_OP_REPLACE:
			status = memcached_replace_by_key(i_obj->obj->memc, server_key, server_key_len, key,
										      key_len, payload, payload_len, expiration, flags);
			break;

		case MEMC_OP_APPEND:
			status = memcached_append_by_key(i_obj->obj->memc, server_key, server_key_len, key,
											 key_len, payload, payload_len, expiration, flags);
			break;

		case MEMC_OP_PREPEND:
			status = memcached_prepend_by_key(i_obj->obj->memc, server_key, server_key_len, key,
											  key_len, payload, payload_len, expiration, flags);
			break;
...
PHP_METHOD(MEMCACHED_CLASS_NAME, cas)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, casByKey)
{
...
static void php_memc_cas_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool by_key)
{
	...
	status = memcached_cas_by_key(i_obj->obj->memc, server_key, server_key_len, key, key_len,
...
PHP_METHOD(MEMCACHED_CLASS_NAME, delete)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, deleteByKey)
{
...
static void php_memc_delete_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool by_key)
{
	...
	status = memcached_delete_by_key(i_obj->obj->memc, server_key, server_key_len, key,
									 key_len, expiration);
...
PHP_METHOD(MEMCACHED_CLASS_NAME, increment)
{
...
PHP_METHOD(MEMCACHED_CLASS_NAME, decrement)
{
...
static void php_memc_incdec_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool incr)
{
	...
	if (incr) {
		status = memcached_increment(i_obj->obj->memc, key, key_len, (unsigned int)offset, &value);
	} else {
		status = memcached_decrement(i_obj->obj->memc, key, key_len, (unsigned int)offset, &value);
	}
...
PHP_METHOD(MEMCACHED_CLASS_NAME, addServer)
{
	char *host;
	int   host_len;
	long  port, weight = 0;
	...
	status = memcached_server_add_with_weight(i_obj->obj->memc, host, port, weight);
	if (php_memc_handle_error(i_obj->obj, status TSRMLS_CC) < 0) {
		RETURN_FALSE;
	}
...
PHP_METHOD(MEMCACHED_CLASS_NAME, addServers)
{
	zval *servers;
	zval **entry;
	...
	status = memcached_server_push(i_obj->obj->memc, list);
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getServerList)
{
	...
	array_init(return_value);
	servers = memcached_server_list(i_obj->obj->memc);
	servers_count = memcached_server_count(i_obj->obj->memc);
	if (servers == NULL) {
		return;
	}
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getServerByKey)
{
	char *server_key;
	int   server_key_len;
	...
	server = memcached_server_by_key(i_obj->obj->memc, server_key, server_key_len, &error);
	if (server == NULL) {
		php_memc_handle_error(i_obj->obj, error TSRMLS_CC);
		RETURN_FALSE;
	}
...
PHP_METHOD(MEMCACHED_CLASS_NAME, getStats)
{
	...
	stats = memcached_stat(i_obj->obj->memc, NULL, &status);
	if (php_memc_handle_error(i_obj->obj, status TSRMLS_CC) < 0) {
		RETURN_FALSE;
	}
	...
	for (i = 0; i < servers_count; i++) {
		hostport_len = spprintf(&hostport, 0, "%s:%u", servers[i].hostname, (unsigned int)servers[i].port);
	...
	memcached_stat_free(i_obj->obj->memc, stats);
}
/* }}} */

/* {{{ Memcached::isPristine()
   Returns true if this object is newly created */
PHP_METHOD(MEMCACHED_CLASS_NAME, isPristine)
{
	MEMC_METHOD_INIT_VARS;
	MEMC_METHOD_FETCH_OBJECT;
...
PHP_METHOD(MEMCACHED_CLASS_NAME, isPersistent)
{
	MEMC_METHOD_INIT_VARS;
	MEMC_METHOD_FETCH_OBJECT;

	RETURN_BOOL(i_obj->obj->is_persistent);
}
/* }}} */

/* {{{ Memcached::getVersion()
   Returns the version of each memcached server in the pool */
PHP_METHOD(MEMCACHED_CLASS_NAME, getVersion)
{
	memcached_server_st *servers;
	unsigned int i, servers_count;
	char *hostport = NULL;
	...
	status = memcached_version(i_obj->obj->memc);
	if (php_memc_handle_error(i_obj->obj, status TSRMLS_CC) < 0) {
		zval_dtor(return_value);
		RETURN_FALSE;
	}
...
static PHP_METHOD(MEMCACHED_CLASS_NAME, flush)
{
	time_t delay = 0;
	memcached_return status;
	MEMC_METHOD_INIT_VARS;
	...
	status = memcached_flush(i_obj->obj->memc, delay);
	if (php_memc_handle_error(i_obj->obj, status TSRMLS_CC) < 0) {
		RETURN_FALSE;
	}
...
static PHP_METHOD(MEMCACHED_CLASS_NAME, getOption)
{
	long option;
	...
	switch (option) {
		case MEMC_OPT_COMPRESSION:
			RETURN_BOOL(i_obj->obj->compression);
		...
		case MEMC_OPT_PREFIX_KEY:
		{
			...
			result = memcached_callback_get(i_obj->obj->memc, MEMCACHED_CALLBACK_PREFIX_KEY, &retval);
		...
		case MEMC_OPT_SERIALIZER:
			RETURN_LONG((long)i_obj->obj->serializer);
			break;

		case MEMCACHED_BEHAVIOR_SOCKET_SEND_SIZE:
		case MEMCACHED_BEHAVIOR_SOCKET_RECV_SIZE:
			if (memcached_server_count(i_obj->obj->memc) == 0) {
		...
		default:
		{
			...
			uint64_t result = memcached_behavior_get(i_obj->obj->memc, flag);
...
static PHP_METHOD(MEMCACHED_CLASS_NAME, setOption)
{
	long option;
	memcached_behavior flag;
	zval *value;
	...
	switch (option) {
		case MEMC_OPT_COMPRESSION:
			...
			i_obj->obj->compression = Z_BVAL_P(value);
			break;
		...
		case MEMC_OPT_PREFIX_KEY:
		{
			...
			if (memcached_callback_set(i_obj->obj->memc, MEMCACHED_CALLBACK_PREFIX_KEY, key) ==
		...
		case MEMCACHED_BEHAVIOR_KETAMA_WEIGHTED:
			...
			if (memcached_behavior_set(i_obj->obj->memc, flag, (uint64_t)Z_LVAL_P(value)) == MEMCACHED_FAILURE) {
			 ...
			 */
			if (!Z_LVAL_P(value)) {
				i_obj->obj->memc->hash = MEMCACHED_HASH_DEFAULT;
				i_obj->obj->memc->distribution = MEMCACHED_DISTRIBUTION_MODULA;
			}
			break;
		...
		case MEMC_OPT_SERIALIZER:
		{
			...
			if (Z_LVAL_P(value) == SERIALIZER_IGBINARY) {
				i_obj->obj->serializer = SERIALIZER_IGBINARY;
			} else
#endif
#ifdef HAVE_JSON_API
			if (Z_LVAL_P(value) == SERIALIZER_JSON) {
				i_obj->obj->serializer = SERIALIZER_JSON;
			} else
#endif

			/* php serializer */
			if (Z_LVAL_P(value) == SERIALIZER_PHP) {
				i_obj->obj->serializer = SERIALIZER_PHP;
			} else {
				i_obj->obj->serializer = SERIALIZER_PHP;
...
static PHP_METHOD(MEMCACHED_CLASS_NAME, getResultCode)
{
	MEMC_METHOD_INIT_VARS;
	...
	RETURN_LONG((long)i_obj->obj->rescode);
}
/* }}} */

/* {{{ Memcached::getResultString()
   Returns the result description from the last operation */
static PHP_METHOD(MEMCACHED_CLASS_NAME, getResultString)
{
	MEMC_METHOD_INIT_VARS;
	...
	switch (i_obj->obj->rescode) {
		// custom error
		case MEMC_RES_PAYLOAD_FAILURE:
			RETURN_STRING("PAYLOAD FAILURE", 1);
		// memcached errors, lazy.. the default
		default:
			RETURN_STRING(memcached_strerror(i_obj->obj->memc, i_obj->obj->rescode), 1);
	}
}
/* }}} */
...
static void php_memc_obj_free(struct php_memc_internal *pi)
{
	if (pi == NULL) {
		return;
	}
...
static void php_memc_free_storage(php_memc_t *i_obj TSRMLS_DC)
{
	...
	if (i_obj->obj && !i_obj->obj->is_persistent) {
		php_memc_obj_free(i_obj->obj);
	}
	efree(i_obj);
}
...
{
	if (rsrc->ptr) {
		struct php_memc_internal *pi = (struct php_memc_internal *)rsrc->ptr;
		php_memc_obj_free(pi);
		rsrc->ptr = NULL;
	}
}
/* }}} */
...
static char *php_memc_zval_to_payload(zval *value, size_t *payload_len, uint32_t *flags, enum memcached_serializer serializer TSRMLS_DC)
{
	...
	if (*flags & MEMC_VAL_COMPRESSED) {
		...
		if (compress((unsigned char *)payload_comp, &payload_comp_len, (unsigned char *)buf.c, buf.len) != Z_OK) {
...
static int php_memc_zval_from_payload(zval *value, char *payload, size_t payload_len, uint32_t flags TSRMLS_DC)
{
	...
	if (flags & MEMC_VAL_COMPRESSED) {
		...
		do {
			...
			status = uncompress((unsigned char *)buf, &length, (unsigned char *)payload, payload_len);
	...
	switch (MEMC_VAL_GET_TYPE(flags)) {
		...
		case MEMC_VAL_IS_SERIALIZED:
		{
			...
			if (!php_var_unserialize(&value, (const unsigned char **)&payload_tmp, (unsigned char *)payload_tmp + payload_len, &var_hash TSRMLS_CC)) {
				ZVAL_NULL(value);
	...
	if (zend_call_function(fci, fcc TSRMLS_CC) == SUCCESS && retval) {
		...
		if (Z_BVAL_P(retval) == 1) {
			...
			if (payload == NULL) {
				...
				rc = memcached_set(i_obj->obj->memc, key, key_len, payload, payload_len, 0, flags);
...
ZEND_BEGIN_ARG_INFO(arginfo_getResultString, 0)
ZEND_END_ARG_INFO()
...
#define MEMC_ME(name, args) PHP_ME(MEMCACHED_CLASS_NAME, name, args, ZEND_ACC_PUBLIC)
static zend_function_entry memcached_class_methods[] = {
    ...
    MEMC_ME(getResultString,    arginfo_getResultString)
...
PHP_MINIT_FUNCTION(memcached)
{
	...
	INIT_CLASS_ENTRY(ce, MEMCACHED_CLASS_NAME_STR, memcached_class_methods);
	memcached_ce = zend_register_internal_class(&ce TSRMLS_CC);
	memcached_ce->create_object = php_memc_new;

    INIT_CLASS_ENTRY(ce, MEMCACHED_EXCEPTION_NAME_STR, NULL);
